...
block.tables = merge({}, block.gfm, {
  ...
  nptable: /^(?: *([^\s\n-].*\|.*)\n)? *([-:]+ *\|[-| :]*)\n((?:.*\|.*(?:\n|$))*)\n*/,
  table: /^(?: *\|([^\n-].*)\n)? *\|( *[-:]+[-| :]*)\n((?: *\|.*(?:\n|$))*)\n*/,
  // Same, with headerless table support and multiline row support
  multilinetable: /^((?: *\|[^\n-].*\n)*) *\|( *[-:]+[-| :]*)\n((?: *\|.*\n)*)( *\|-[-|]* *\n)((?: *\|.*(?:\n|$))*)\n*/
  // --- cronvel
});
...
Lexer.prototype.token = function(src, top, bq) {
  ...
  while (src) {
    ...
    // +++ cronvel
    // multi-line table -- MUST BE BEFORE: table(gfm)
    if (top && (cap = this.rules.multilinetable.exec(src))) {
      src = src.substring(cap[0].length);
      
      if ( cap[4] ) { cap[3] += cap[4] }
      ...
      for (i = 0 ; i < rows.length; i++) {
        rows[i]
          .replace(/^ *\| *| *\| *$/g, '')
          .split(/ *\| */)
          .forEach( (v , k) => item.header[k] = item.header[k] ? item.header[k] + ' ' + v : v );
      }
      ...
      for (i = 0 , j = 0 ; i < rows.length; i++) {
        ...
        if ( i && rows[i].match( /^ *\|-[-|]* *$/ ) ) {
          ...
          rows[i]
            .replace(/^ *\| *| *\| *$/g, '')
            .split(/ *\| */)
            .forEach( (v , k) => item.cells[j][k] = item.cells[j][k] ? item.cells[j][k] + ' ' + v : v );
        }
      }
    ...
    if (top && (cap = this.rules.table.exec(src))) {
      ...
      for (i = 0 , j = 0 ; i < item.cells.length; i++ , j++) {
  ...
  switch (this.token.type) {
    ...
    case 'table': {
      ...
      if ( this.token.header) {
        ...
        if ( this.token.header.length === 1 && this.token.align.length > 1 ) {
          cell += this.renderer.tablecell(
            this.inline.output(this.token.header[0]),
            { header: true, align: 'center', colspan: this.token.align.length }
          );
        }
        else {
